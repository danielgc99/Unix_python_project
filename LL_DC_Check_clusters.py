...
def create_filtered_distance_matrix(point_data, quality_threshold):
    ...
    print("Closest pairs (showing up to 10):")
    for pair in closest_pairs[:10]:  # Show first 10 pairs
        print(f"  {point_data[pair[0]][0]} and {point_data[pair[1]][0]}")
    if len(closest_pairs) > 10:
        print(f"  ... and {len(closest_pairs) - 10} more pairs")
...
def update_candidate_clusters_with_diameter_cache(candidate_clusters, best_cluster, distance_matrix, threshold, used_points, point_neighbors, point_data):
    ...
    if len(removed_centers) > 0:
        display_centers = [point_data[idx][0] for idx in removed_centers[:5]]
        if len(removed_centers) > 5:
            display_centers.append(f"... and {len(removed_centers) - 5} more")
    ...
    if len(regenerated_centers) > 0:
        display_regen = [point_data[idx][0] for idx in regenerated_centers[:5]]
        if len(regenerated_centers) > 5:
            display_regen.append(f"... and {len(regenerated_centers) - 5} more")
